...
interface UseAutoRestartOptions {
  ...
  initialDelay?: number; // Initial delay before starting countdown (default 0)
 ...
 * @param options.initialDelay - Initial delay before starting countdown (default: 0)
 ...
 *   initialDelay: 5000, // Wait 5 seconds before starting countdown
...
export function useAutoRestart({
  ...
  useEffect(() => {
    if (winner) {
      ...
      // Delay starting the countdown by initialDelay milliseconds
  ...
  }, [winner, onRestart, setCountdown, duration, initialDelay, currentUserId, players]);
}
