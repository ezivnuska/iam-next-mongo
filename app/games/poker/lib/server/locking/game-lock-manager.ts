...
import { initializeBets } from '@/app/games/poker/lib/utils/betting-helpers';
import { withRetry } from '@/app/lib/utils/retry';
import { POKER_RETRY_CONFIG } from '@/app/games/poker/lib/config/poker-constants';
import { acquireGameLock, releaseGameLock } from './game-lock-utils';
import { logGameStartedAction } from '@/app/games/poker/lib/utils/action-history-helpers';
      ...
      try {
        ...
        for (const player of gameToLock.players) {
          if ((player as any).chips && Array.isArray((player as any).chips)) {
            ...
            const { POKER_GAME_CONFIG } = await import('@/app/games/poker/lib/config/poker-constants');
