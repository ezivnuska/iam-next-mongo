// app/api/mobile/issues/[id]/commission/abstain/route.ts
// POST — author or contributor abstains from rating (stores score: null).
// Abstentions count toward "all responded" but are excluded from the average.
// After all required raters (author + pledgers) have responded, auto-approves if avg ≥ 4, else denies.
...
export const POST = withAuth(async (req, token, ctx) => {
  ...
  try {
    ...
    if (!isAuthor) {
      ...
      if (!pledge) return NextResponse.json({ error: 'Only the author or contributors can abstain' }, { status: 403 })
    }
    ...
    await Rating.findOneAndUpdate(
      ...
      { issueId, commissionId, raterId: token.id, workerId: acceptedApplicant.userId, score: null },
      { upsert: true }
    )
    ...
    if (autoDecision === 'approved') {
      ...
      if (claimed) {
        try { await settleIssue(issueId) } catch (err) {
          console.error('[commission/abstain] settleIssue failed:', err)
        }
      }
    ...
    console.error('[commission/abstain POST]', err)
    return NextResponse.json({ error: 'Failed to submit abstention' }, { status: 500 })
  }
})
