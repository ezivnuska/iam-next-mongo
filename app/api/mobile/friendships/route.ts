...
export const POST = withAuth(async (req, token) => {
  try {
    ...
    if (existing) {
      ...
      if (status === 'accepted') return NextResponse.json({ error: 'Already friends' }, { status: 400 })
      if (status === 'pending') return NextResponse.json({ error: 'Friend request already pending' }, { status: 400 })
      if (status === 'rejected') {
        ...
        return NextResponse.json({ friendship: { id: (existing as any)._id.toString(), status: 'pending_sent' } })
      }
    }
    ...
    return NextResponse.json({ friendship: { id: (friendship as any)._id.toString(), status: 'pending_sent' } }, { status: 201 })
  } catch (err) {
